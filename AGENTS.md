...
- The scheduler is a core primitive for local AI workbench automation. It should run selected skills/workflows through selected harnesses against explicit scopes, permissions, output policies, logs, artifacts, provenance, and review state.
- Skills/workflows are run instructions or commands executed by a selected harness. Harnesses are integrations/plugins; skills may be shipped by plugins or profiles but should not be treated as separate worker runtimes by default.
