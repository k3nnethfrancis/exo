...
- When Fable architectural review is needed, use the Exo-managed Claude Code session named `exo-fable-oracle`; `communications.md` is historical context only. Public agent/operator contract changes still require Fable/architect review before shipping unless the user explicitly approves an exception.
