...
- Exo uses an orchestrator-oracle coding pattern. The lead/orchestrator decomposes work, manages subagents, reviews/merges, and escalates scarce architecture questions to Fable through the Exo-managed Claude Code session named `exo-fable-oracle`; `communications.md` is historical context only. Use the local `exo-fable-oracle` Codex skill for when to reuse, compact, or start a fresh Fable session. Subagents must not call Fable directly. They should stop before architectural/public-contract changes, report evidence/options/recommendations to the orchestrator, and wait for steer. Public agent/operator contract changes still require Fable/architect review before shipping unless the user explicitly approves an exception.
