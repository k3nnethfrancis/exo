...
| --- | --- | --- |
| Onboarding | Guided first-run profile building: workspace paths, notes folder, recommended official plugin choices, detected harness choices, starter routine templates, dynamic starter agent context, standard skill review, and profile name/review before entering the workspace. | Long-term editing, plugin lifecycle administration, instruction-file editing, skill installation, routine management, permission grants, or hidden file writes. |
...
| Plugin Manager | Optional capability lifecycle: discovery, trust, enable/disable, dependency/setup state, permissions, plugin-owned settings, local plugin folder management, search providers such as QMD, harness adapters, profile packs, analyzers, dashboards, and routine-template plugins. Core substrate is summarized, not listed as an optional plugin row. | Active profile editing, core substrate configuration, concrete routine runs/schedules, or agent instruction file content. |
