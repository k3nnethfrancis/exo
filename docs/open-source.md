...
Electron can support the other platforms later, but Exo's direct-PTY terminal runtime,
native dependencies, and agent recovery flows need explicit Windows/Linux validation
...
`pnpm check` remains the unused-symbol/typecheck/test/build subset. The package workflow is manual and builds unsigned macOS artifacts.
