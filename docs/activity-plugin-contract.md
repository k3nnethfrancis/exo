...
Dogfooding retention policy: semantic trace files are private workspace-runtime artifacts and are session-isolated by the requested session id. CLI, MCP, and command-server answer reads inspect a bounded event tail by default, currently 100 events, so a read should not summarize or merge unrelated sessions. Disk retention is intentionally manual for the pre-public dogfooding phase: normalized traces and raw sidecars remain under `.exo/traces/` until the operator removes retired session files or clears the traces directory. Before broad real-vault plugin dogfooding, Exo should add a settings-visible trace retention or cleanup policy; do not add hidden age, size, or count caps that silently delete trace evidence.
