...
| Invocation Command setup | shipped | onboarding in `App.tsx`; `WorkspaceSettingsDialog.tsx`; `agent-invocation.ts` | `agent-invocation.spec.ts`; core invocation tests | `README.md`; `docs/invocation-context-and-safety.md` | Required | Keep recommended defaults editable; keep prompt editing advanced rather than primary. |
