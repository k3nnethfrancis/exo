...
| Typed knowledge-graph contract, evidence, and relation origins | shipped foundation | `knowledge-graph.ts`; `workspace-graph.ts` | graph integrity/projection/query/snapshot/workspace tests | graph system report; ADR 0005 and 0006 | Required foundation | Public Knowledge Graph 0.3 uses `document | ontology | inferred`; bounded internal Format compatibility names remain until the Note Root Format migration. |
| Internal Generic Markdown and OKF Format interpreters | experimental | `knowledge-profile.ts`; workspace graph | workspace graph/Format tests | graph system report; ADR 0005 | Foundation only | Format selection remains internal and defaults to `generic-markdown`. Workspace Ontology review is now a separate shipped surface; do not market OKF as user-configurable until Note Root Format settings exist. |
