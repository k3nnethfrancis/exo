...
| Note-local graph neighborhood | shipped | `GraphNeighborhoodView`; graph context/query; shared graph scene/presentation compiler/Canvas renderer | neighborhood presentation, graph query, and Electron shell tests | graph system report | Important | Keep it bounded and passive; exact worker-relaxed coordinate continuity with the full Pane remains deliberately out of scope. |
...
| Typed knowledge-graph contract, evidence, and relation origins | shipped foundation | `knowledge-graph.ts`; `workspace-graph.ts` | graph integrity/projection/query/snapshot/workspace tests | graph system report; ADR 0005 and 0006 | Required foundation | Public Knowledge Graph 0.3 uses `document | ontology | inferred`; the closed internal Format migration is complete. |
| Internal Generic Markdown and OKF Format interpreters | internal foundation | `note-root-format.ts`; `WorkspaceGraph` | Note Root Format, workspace graph, and interoperability tests | graph system report; ADR 0005 | Foundation only | Production owns Generic Markdown; canonical `okf` selection exists only through the explicit interoperability-fixture constructor. No renderer, IPC, or settings selector is shipped. |
...
| User-owned Ontology library and reviewed activation | shipped early access | `WorkspaceOntologyStore`; root + flat library sources; serialized review worker; Settings/Graph selector | ontology/parser/store/graph/watcher tests; stale→Keep→restart→Generic tests | `workspace-ontology.md`; ADR 0006 and 0007 | Pre-dogfood loop | One source or Generic is active; every switch previews effects and requires explicit Keep. |
| Ontology discovery/design Skill | shipped early access | disposable Markdown snapshot; provider-specific read-only Claude/Codex runner; host-only staging | structured-response, snapshot-isolation, stale-identity, and store staging tests | ontology productization plan; spec 10 | Owner-reviewed dogfood, not an automated quality gate | Discovery may stage only root `ontology.yaml`; malformed, stale, failed, question, and abstain paths do not mutate live Notes or Active. |
| Ontology graph-maintenance Skill | shipped first Skill | user-owned Skill; graph-selected action; ordinary inline Invocation and Changeset | Skill containment/revision tests; invocation/Changeset suites | graph-system plan; spec 09 | Add another Skill only after dogfood | Exact Skill, active Ontology, and graph snapshot identities travel with the Invocation record. |
