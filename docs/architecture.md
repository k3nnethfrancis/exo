...
| Exo MCP retrieval | `packages/cli/src/mcp-server.ts`, `provider-mcp-setup.ts` | Optional provider-owned install of Exo's frozen read-only status/search/read tools; public stabilization requires caller-cwd scope rather than app-active scope | MCP + provider-setup tests; `provider-mcp-onboarding.md`, `reviews/2026-07-13-fable-mcp-agent-context-packet.md` |
