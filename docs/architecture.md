...
| Exo MCP retrieval | `packages/cli/src/mcp-server.ts`, `provider-mcp-setup.ts` | Optional provider-owned install of Exo's read-only Workspace status/search/read server | MCP + provider-setup tests; `provider-mcp-onboarding.md` |
