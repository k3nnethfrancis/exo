...
Start with the smallest repair. Your Markdown is canonical; Exo's indexes,
...
If QMD is catching up or recovering, Exo keeps foreground retrieval available
...
is installed and reachable by the desktop app. Exo asks for explicit trust when
...
- The selected provider's own CLI must be installed before Exo can add its MCP
  configuration.
- An existing provider registration means the server is already installed; MCP
  setup does not replace the local `exo` launcher.
