...
test("boots the shell, opens notes, and manages terminal tabs", async () => {
  ...
  await expect(page.locator('section[data-testid="markdown-graph-references"]')).toContainText("agent-memory");

  await page.getByTestId("markdown-graph-backlinks").getByText("Related Note").click();
...
test("renders inline graph references without inspector chrome", async () => {
  const { page, cleanup } = await launchExoFixture();
  const graphReferences = page.locator('section[data-testid="markdown-graph-references"]');

  await expect(page.getByTestId("inspector-toggle")).toHaveCount(0);
  await expect(graphReferences).toContainText("Backlinks");
  await expect(graphReferences).toContainText("References");
  await expect(graphReferences).toContainText(/Related Note|agent-memory/);
