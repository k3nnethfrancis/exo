...
async function openRelatedNoteViaInlineBacklink(page: import("@playwright/test").Page) {
  const relatedBacklink = page.getByTestId("markdown-graph-backlinks").getByRole("button", { name: "Related Note" });
  await relatedBacklink.scrollIntoViewIfNeeded();
  await relatedBacklink.click();
}
...
test("boots the shell, opens notes, and manages terminal tabs", async () => {
  ...
  await expect(page.getByTestId("markdown-graph-references").first()).toContainText("Backlinks");
  await expect(page.getByTestId("markdown-graph-references").first()).toContainText("References");
  ...
  await page.getByTestId("toggle-markdown-mode").click();
  await openRelatedNoteViaInlineBacklink(page);
...
test("renders inline graph references without an inspector action", async () => {
  ...
  await expect(page.getByTestId("markdown-graph-references").first()).toContainText("agent-memory");
