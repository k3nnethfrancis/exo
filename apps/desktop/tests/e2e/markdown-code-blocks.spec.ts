...
`;
  ...
  await expect.poll(() => page.locator(".exograph-md-line--codeblock").count()).toBeGreaterThanOrEqual(6);
  await expect(page.locator(".exograph-md-line--codeblock-start")).toHaveCount(3);
  await expect(page.locator(".exograph-md-line--codeblock-end")).toHaveCount(3);
  await expect(page.locator(".exograph-md-line--codeblock .exograph-md-tag")).toHaveCount(0);
  await expect(page.locator(".exograph-md-line--codeblock .exograph-md-link")).toHaveCount(0);

  const longCodeLine = page.locator(".exograph-md-line--codeblock").filter({ hasText: /^cover cover/ }).first();
  ...
  const finalCodeLine = page.locator(".exograph-md-line--codeblock-end").last();
  const ruleLine = page.locator(".exograph-md-line--rule").first();
