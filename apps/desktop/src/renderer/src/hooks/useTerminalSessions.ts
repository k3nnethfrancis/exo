...
export function useTerminalSessions(options: UseTerminalSessionsOptions) {
  ...
  useEffect(() => {
    const removeDataListener = window.exograph.terminals.onData(({ id, generation, data }) => {
    ...
    const removeExitListener = window.exograph.terminals.onExit(({ id, exitCode }) => {
      setSessions((current) =>
        current.map((session) => (session.id === id ? { ...session, status: "exited", exitCode } : session)),
      );
    });
    const removeCreatedListener = window.exograph.terminals.onCreated((session) => {
      adoptExternalSessions([session], { activateLatest: true });
    });
    const syncInterval = window.setInterval(() => {
      void window.exograph.terminals.list().then((nextSessions) => {
  ...
  async function hydrateTerminal(id: string, options?: { force?: boolean; reason?: TerminalHydrationReason }) {
    ...
    try {
      const snapshot = await window.exograph.terminals.read(id);
  ...
  async function hydrateTerminalAfterGenerationSync(id: string, generation: number): Promise<void> {
    ...
    try {
      const nextSessions = await window.exograph.terminals.list();
  ...
  async function createTerminal(terminalKind: TerminalKind, cwd?: string): Promise<TerminalSessionInfo> {
    const session = await window.exograph.terminals.create({ terminalKind, cwd });
  ...
  async function killTerminal(id: string): Promise<TerminalSessionInfo[]> {
    await window.exograph.terminals.kill(id);
