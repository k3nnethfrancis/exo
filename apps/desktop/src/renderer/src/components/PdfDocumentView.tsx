...
export function PdfDocumentView({ filePath }: PdfDocumentViewProps) {
  ...
  return (
    <section className="pdf-document-view" data-testid="pdf-document-view">
      <div className="pdf-document-view__toolbar" aria-label="PDF controls">
        <button aria-label="Previous page" className="browser-pane__button" disabled={pageNumber <= 1} onClick={() => setPageNumber((page) => previousPdfPage(page))} title="Previous page" type="button"><ChevronLeft size={15} /></button>
        <span aria-label={`Page ${pageNumber} of ${document.numPages}`} className="pdf-document-view__page-count">{pageNumber} / {document.numPages}</span>
        <button aria-label="Next page" className="browser-pane__button" disabled={pageNumber >= document.numPages} onClick={() => setPageNumber((page) => nextPdfPage(page, document.numPages))} title="Next page" type="button"><ChevronRight size={15} /></button>
