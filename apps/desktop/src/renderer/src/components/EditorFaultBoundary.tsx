 ...
 * is written to Exograph's local log for a real reproduction/fix cycle.
 */
export class EditorFaultBoundary extends Component<EditorFaultBoundaryProps, EditorFaultBoundaryState> {
  ...
  componentDidCatch(error: Error, _info: ErrorInfo): void {
    ...
    console.error("[exograph] editor render fault", diagnostic, error);
    void window.exograph.workspace.recordRendererDiagnostic(diagnostic).catch((loggingError) => {
      console.error("[exograph] failed to persist editor render diagnostic", loggingError);
    });
  }
    ...
    if (this.state.hasFault) {
      return (
        <section className="editor-fault" role="status" aria-live="polite">
          ...
          <span>Refresh Exograph to continue.</span>
        </section>
      );
    }
