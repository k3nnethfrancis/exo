...
async function listAgentInstructionOverlays() {
  const overlays = agentInstructionOverlays();
...
function agentInstructionOverlays() {
  const overlayRoots = [
    {
      id: "global",
  ...
  return overlayRoots.map((overlay) => ({
    ...
    label: overlay.label,
    path: overlay.path,
    body: renderAgentInstructionOverlay(overlay.label, overlay.scope, overlay.targetRootPath),
  }));
}

function renderAgentInstructionOverlay(label: string, scope: "global" | "notes" | "project", targetRootPath: string) {
  return [
    "# Exo Runtime Context",
    "",
