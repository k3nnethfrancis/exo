...
export interface WorkspaceIpcHandlers {
  ...
  markOnboardingProfileSetup: WorkspaceApi["markOnboardingProfileSetup"];
...
export function registerWorkspaceIpcHandlers(handlers: WorkspaceIpcHandlers) {
  ...
  handleDesktopInvoke("workspace:mark-onboarding-profile-setup", async (_event, input) => handlers.markOnboardingProfileSetup(input));
