...
- Added the first Routine CLI MVP: `exo routines templates`, `exo routines list`, `exo routines create`, and explicit `exo routines run --dry-run` operate through a core `RoutineService` and the `.exo/routines` / `.exo/runs` store without launching agents yet.
