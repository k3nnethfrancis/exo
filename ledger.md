...
- Fixed markdown editor QoL regressions: clean-file refreshes now restore cursor selection instead of jumping to the top, live-preview bullets/numbered lists continue on Enter, empty list items exit cleanly, cursor filtering avoids hidden list-marker positions, and Tab/Enter exits `[[wikilinks]]` to a following space for continued inline typing.
