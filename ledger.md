...
- WP-046 MCP stdio restart/reinstall diagnosis closed: direct stdio from the expected launcher was healthy, while local Codex/Claude host config pointed at a stale checkout. `exo integrations doctor` now reports stale MCP launcher paths, `exo integrations test` fails them, and `exo integrations install` replaces stale entries before users restart/refresh existing agent sessions.
...
- WP-P4b landed the first production semantic trace producer: Pi-compatible harness launch plans carry a `sidecar-jsonl` declaration, desktop provisions and ingests `.exo/traces/sidecars/{sessionId}.ndjson` into `.exo/traces/{sessionId}.ndjson`, and CLI/MCP read messages now distinguish transcript/live-tail evidence from trace-backed semantic answers.
...
- Removed rapid-backspace stalls in large Markdown notes: live preview no longer rebuilds whole-note structural metadata for edits to ordinary list-item text, title derivation is skipped when the opening title source is unchanged, and the editor latency suite now stress-tests multiline list deletion.
