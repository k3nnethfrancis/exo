...
- Makes onboarding configuration trustworthy: Agent Context now previews the actual global and active-notes `AGENTS.md` / `CLAUDE.md` files, Exograph context apply refreshes visible file previews, instruction-file merge uses a deterministic local operation, Skills onboarding installs/enables bundled Exo skills through the shared Agent Config skill service, and Profile setup exposes direct profile name/config editing with immediate save.
