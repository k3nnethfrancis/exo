...
- Adds optional Ontology discovery through a trusted Claude or Codex Command.
  ...
  controls; Exograph validates and stages a proposal, but never activates it without
  Keep.
...
- Adds `Mod+B` and `Mod+Alt+B` shell shortcuts for Explorer and Utility,
  plus a compact lower-menu Help surface sourced from Exograph's current
...
- Adds automatic semantic catch-up for QMD `On save` indexing: small pending
  sets run in bounded slices only after Exograph is quiet and the system is idle,
  while `Manual only` remains an explicit pause.
- Adds Exograph-aware inline Command prompts, snapshot-backed inline editor diffs,
  and per-Command Claude context continuity with visible provenance and reset.
- Adds a focused first-run flow: choose one main wiki, optionally install Exograph's Workspace status/search MCP tools into Claude and Codex, then persist editable local invocation commands.
...
- Prevents a packaged app's bundled plugin resources from being mistaken for a
  source checkout when Exograph reports CLI installation status.
...
- Makes CLI and MCP search return the shared bounded `exograph.search.v1` page with
...
- Narrows the optional Exograph MCP from status/search/read to workspace status and search only; agents use returned paths through their own native file permissions.
- Resolves Exograph MCP scope from the provider process's caller directory, refuses ambiguous Workspace matches, and falls back to scoped filesystem retrieval when a running app belongs elsewhere.
...
- Distinguishes a headless Command's chat/stdout from the Exograph note in the invocation prompt, requires a successful filesystem write for linked responses, and fails stdout-only protocol completions that never reach the document.
- Starts with an empty editor when no saved layout chooses a note instead of hard-coding an Exograph `tasks.md`; restores only user-selected tabs and migrates saved canvas layouts from v2 to the renderer's canonical v3 schema.
...
- Diagnoses the CLI visible to the desktop app during onboarding—current checkout, legacy Exograph shim, missing command, or a command owned by something else—and offers an explicit checkout install/update command without ever changing it from MCP setup.
...
Installed-app readiness, direct-pty terminal reliability, Exograph-on-Exograph harness cleanup, and resident runtime support.
...
- Adds `pnpm dev:qa` so source QA uses isolated `.exograph-dev/` runtime and user-data state while installed Exograph keeps coordinating notes, agents, CLI, and MCP.
- Adds a resident runtime menu-bar controller: closing the window hides Exograph, the process keeps the command server/MCP/watchers/transcripts/live pty agents alive, and explicit Quit warns before stopping live terminals.
...
- Installs and verifies `/Applications/Exograph.app` as the local stable runtime with a visible macOS menu bar icon; normal `exograph status` resolves to the installed runtime while `pnpm dev:qa` can run side-by-side.
- Tracks the remaining CI Node 20 GitHub Actions deprecation warning as `EXOGRAPH-ISSUE-025`.

## 0.1.0-alpha.2 - 2026-05-17

Tester-readiness hardening for terminal history, Markdown tasks, and agent search.

- Simplifies terminal history behavior around explicit `full` and `custom` modes: `full` keeps Exograph buffers untrimmed and transcripts default to forever.
...
- Adds Exograph-managed QMD indexing as an active optional substrate with lexical, semantic, and hybrid modes, Settings controls, status UI, sync/update/embed actions, and Explore indexed search on Enter.
- Exposes QMD-backed index status, search, read, sync, update, and embed flows through the Exograph CLI/MCP command server while keeping terminal-agent CLI/MCP tools intact.
...
- Defines Exograph as a local-first agentic development environment built around a shared exocortex.
...
- Adds Exograph CLI and MCP control surfaces for live terminal agents.
