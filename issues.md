...
- Status: fixed in draft PR for GitHub issue #33
- Severity: medium
...
- Observed:
  - Entering Monitor Mode with an existing session list and adding sessions while Monitor Mode was already enabled used different layout paths.
  - The live-add path wrapped the target terminal leaf in a fixed horizontal split.
...
- Acceptance:
  - [x] Define expected split-tree shapes for 1, 2, 3, 4, 5, 6, and 8 terminal sessions.
...
- Fix notes:
  - Monitor Mode now uses a deterministic row/column grid with proportional split ratios, and live additions rebuild through the same builder as initial Monitor Mode entry.
  - The refreshed production Electron QA also caught a terminal mount crash from xterm's Unicode 11 addon requiring `allowProposedApi`; terminal construction now enables that API so the Unicode width addon can load.
  ...
  - Verification: `pnpm stable:smoke` passed all 12 scenarios on 2026-07-07.
...
- Acceptance:
  - [x] Define stable sizing rules for Settings and onboarding dialogs.
  - [x] Keep the outer modal/window frame stable across Settings tab changes.
  - [x] Keep the outer modal/window frame stable across onboarding step changes.
  - [x] Use internal scrolling or responsive two-column layouts instead of resizing the whole modal around content.
  - [x] Add visual/e2e coverage for switching Settings tabs and onboarding steps without outer-frame jumps.
  - [x] Include packaged-app QA screenshots or video evidence for desktop dimensions.
    - 2026-07-06: focused Electron e2e captures `/tmp/exo-issue-32-settings-tabs.png`, `/tmp/exo-issue-32-onboarding-plugins.png`, and `/tmp/exo-issue-32-onboarding-review.png`; `pnpm pack:mac` completed successfully.
    - Fresh human packaged-app QA is still useful because this is a visual feel issue.
...
- Acceptance:
  - [x] Document which concepts belong to Onboarding, Settings, Plugin Manager, Agent Config, Routine Manager, and Profile Manager.
    - Added `docs/onboarding-settings-boundaries.md` and linked it from the docs map and plugin architecture doc.
  - [x] Update Settings search/index UI so it reflects the plugin/provider architecture instead of treating QMD as native-only Exo behavior.
    - Workspace Settings now frames Search as core search plus QMD advanced provider and links provider lifecycle to Plugin Manager.
  - [x] Add a clear resume/finish setup path when onboarding is incomplete.
    - Incomplete profile setup appears as a `Finish setup` bottom-bar action.
  - [x] Add a clear active workspace profile surface after onboarding is complete.
  - [x] Ensure onboarding choices have corresponding post-onboarding management surfaces.
  - [x] Add tests or QA scripts for the transition from onboarding to Settings/Profile management.
    - Renderer tests cover the Search/QMD copy; focused e2e covers incomplete setup resume and completed profile state.
...
- Acceptance:
  - [x] Add explicit persisted state for onboarding progress/completion separate from workspace settings existence.
    - Added `onboarding-state.json` under app user data with typed core helpers and desktop IPC.
  ...
  - [x] Add packaged-app QA covering refresh/relaunch during each onboarding stage.
    - 2026-07-06: focused Electron e2e covers hard reload during profile setup and workspace-settings-only partial setup; `pnpm pack:mac` completed successfully.
    - Fresh human packaged-app QA is still recommended before considering first-run onboarding done.
...
- Resolution:
  ...
  - 2026-07-03: Added the semantic trace v1 envelope and optional harness trace declarations so future harness capture/export plugins can emit structured events without touching terminal rendering.
...
- Resolution notes:
  ...
  - 2026-07-04 Wave-4 fan-out: fresh Exo-launched Codex workers in isolated worktrees again showed `MCP client for exo failed to start ... connection closed: initialize response`, while CLI/app terminal creation worked. This keeps live restart/reinstall QA on the plugin dogfooding critical path.
  - 2026-07-05 follow-up fix: Exo-launched Codex sessions now build their MCP override from the imported/running Exo project root before the agent cwd. This prevents isolated worktrees from pointing Codex at an unbuilt worktree-local `packages/mcp/bin/exo-mcp.mjs`. Live restart QA launched `term-130` from `/Users/kenneth/Desktop/lab/projects/exo-worktrees/wp-exo-mcp-live-restart`; its transcript no longer showed the Exo MCP startup warning, and process inspection showed the MCP server running from `/Users/kenneth/Desktop/lab/projects/exo/packages/mcp/bin/exo-mcp.mjs`.
  - 2026-07-05 verification in `codex/wp-exo-mcp-live-restart`: `codex mcp get exo` reported command `/opt/homebrew/Cellar/node/25.8.1/bin/node` with args `/Users/kenneth/Desktop/lab/projects/exo/packages/mcp/bin/exo-mcp.mjs`, and process inspection of Exo-created Codex sessions showed `mcp_servers.exo.args=["/Users/kenneth/Desktop/lab/projects/exo/packages/mcp/bin/exo-mcp.mjs"]` plus the same imported-root MCP child process. No worktree-local MCP launcher path was present.
