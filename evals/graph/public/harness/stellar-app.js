...
async function boot() {
  if (coarsePointer) {
    ...
    shell.setAttribute('aria-label', 'Exograph spatial graph. Drag to orbit, use two fingers to pan, and pinch to zoom.');
  }
  ...
  window.__exographStellarLab = {
    ...
    actions: {
      ...
      exerciseRendererRecovery: async () => {
        await handleRendererFailure(new Error('Graph evaluation injected renderer failure'), state.rendererGeneration);
        await handleRendererFailure(new Error('Graph evaluation injected renderer failure'), state.rendererGeneration);
