...
import { presentationProfileHash, resolvePresentationProfile } from './public/harness/stellar-scene.js';

const graphRoot = path.dirname(fileURLToPath(import.meta.url));
...
const runDirectory = path.join(graphRoot, 'artifacts', runId);
const fixtureDirectory = path.join(graphRoot, 'artifacts', 'fixtures');
...
const server = await startServer({ root: repositoryRoot });
const results = [];
let hardware = null;

try {
  const stampPage = await context.newPage();
  await stampPage.goto('about:blank');
    ...
    for (const engine of profile.engines) {
      ...
      for (const track of profile.tracks) {
        for (let trial = 1; trial <= repetitions; trial += 1) {
          ...
          const result = await runCase({ context, server, adapter, track, fixture, trial, presentationProfile: profile.presentationProfile || 'evaluation-v1' });
          results.push(result);
...
await fs.writeFile(path.join(graphRoot, 'artifacts', 'latest.json'), `${JSON.stringify(report, null, 2)}\n`);
await fs.writeFile(path.join(graphRoot, 'artifacts', 'latest.md'), renderMarkdown(report));
