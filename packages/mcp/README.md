...
- `read_agent` — read bounded live terminal tail output by default. Prefer `maxLines` for line-bounded live tails; `tailChars` reads transcript tails. ANSI cleanup is enabled by default. Use `source: "trace"` to read persisted semantic agent answer text from `.exo/traces/{agentId}.ndjson`; this is the durable path for TUI harnesses whose displayed answer can be repainted away.
