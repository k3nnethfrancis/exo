...
`@exo/core` owns portable domain rules and shared data contracts. It must stay
...
```bash
pnpm --filter @exo/core exec vitest run src/__tests__/workspace-files.test.ts
pnpm --filter @exo/core exec vitest run src/__tests__/workspace-graph.test.ts src/__tests__/ontology-review.test.ts
