...
These fixtures freeze the smallest public inputs needed to verify Exograph's OKF
...
that omission is the only Exograph-side change. The source is Apache-2.0 and the
...
`format-ontology-boundary/` is Exograph-authored test data. It proves that OKF
