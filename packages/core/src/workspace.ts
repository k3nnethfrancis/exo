...
export function resolveWorkspaceModel(env: NodeJS.ProcessEnv = process.env): WorkspaceModel {
  const workspaceRoot = env.EXO_WORKSPACE_ROOT ?? env.STEM_WORKSPACE_ROOT ?? process.cwd();
  const defaultTerminalCwd = env.EXO_DEFAULT_TERMINAL_CWD ?? env.STEM_DEFAULT_TERMINAL_CWD ?? workspaceRoot;
  const noteRootCandidates = (env.EXO_NOTE_ROOTS ?? env.STEM_NOTE_ROOTS ?? path.join(workspaceRoot, "notes")).split(path.delimiter).filter(Boolean);
  const indexedRoots = parseIndexedRoots(env.EXO_INDEXED_ROOTS ?? env.STEM_INDEXED_ROOTS);
...
function parseIndexingConfig(env: NodeJS.ProcessEnv): IndexingConfig {
  const mode = normalizeIndexMode(env.EXO_INDEX_MODE ?? env.STEM_INDEX_MODE);
  return {
    enabled: (env.EXO_INDEX_ENABLED ?? env.STEM_INDEX_ENABLED) === "1" || mode !== "off",
    mode,
    backend: "qmd",
  };
}
