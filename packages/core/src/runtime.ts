...
export function renderPrimaryAgentInstructions(config: RuntimeConfig): string {
  ...
  return [
    ...
    "- Use the `exo` CLI as the local operator and debug surface when you need Exo workspace state.",
    ...
    "- guidance: use `exo search` and `exo read` for Exo-managed knowledge index access.",
    "",
    "## Agent Communication",
