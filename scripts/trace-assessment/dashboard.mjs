...
<section class="panel"><h2>Recurring features</h2><p class="note">Pairwise Jaccard compares exact normalized strings, so naming differences lower the score. Inter-run agreement is exploratory, not a quality score.</p>${matrixTable(matrix.runs, recurringRows)}<details class="one-offs"><summary>One-off features · ${oneOffRows.length}</summary>${matrixTable(matrix.runs, oneOffRows)}</details></section>
...
function featureSummary(features){if(!features)return'';const items=[...features.conceptTypes.map((item)=>`type:${item}`),...features.relations.map((item)=>`relation:${item}`)];return items.length===0?'':`<div class="feature-summary">${items.map((item)=>`<span class="feature-pill">${escapeHtml(item)}</span>`).join('')}</div>`}
function proposalList(label,items=[],render){return items.length===0?'':`<details class="proposal-detail"><summary>${label} · ${items.length}</summary><ul>${items.map((item)=>`<li>${render(item)}</li>`).join('')}</ul></details>`}
function matrixTable(runs,rows){return `<div class="matrix-wrap"><table><thead><tr><th>Feature</th>${runs.map((run)=>`<th>${escapeHtml(run)}</th>`).join('')}</tr></thead><tbody>${rows.map((row)=>`<tr><th>${escapeHtml(row.feature)}</th>${row.present.map((present)=>`<td>${present?'●':'·'}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`}
